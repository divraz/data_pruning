...
def load_and_filter_dataset (dataset_name):                                     
  ...
  print (file_name)
  xyz = pd.read_pickle (model_save_path + 'embeddings/' + file_name + '.pkl')
  index_ids = xyz.index.tolist ()                                             
                                                                                
  dataset['train'] = dataset['train'].filter(lambda example: example['indexes'] in index_ids)
  dataset['train'] = dataset['train'].remove_columns(["indexes"])  
  dataset['train'] = dataset['train'].add_column ('indexes', [i + 1 for i in range( len (dataset['train'] ))])
  print (dataset)
  return dataset
...
def model_training (model, epochs, tokenized_datasets, batch_size):
  ...
  val_loader = DataLoader (tokenized_datasets['validation_matched'],
                          sampler = SequentialSampler(tokenized_datasets['validation_matched']),
                          batch_size = batch_size)
  test_loader = DataLoader (tokenized_datasets['validation_mismatched'],
                          sampler = SequentialSampler(tokenized_datasets['validation_mismatched']),
                          batch_size = batch_size)
  ...
  for epoch_i in tqdm(range(0, epochs)):
      ...
      [total_eval, total_eval_accuracy, total_eval_loss, predictions, true_labels] = train (model, optimizer, scheduler, test_loader, epoch_i, eval = True)
      if epoch_i == 0:
  ...
  [test_metrics['total'], test_metrics['accuracy'], test_metrics['loss'], test_metrics['predictions'], test_metrics['true_labels']] = train (model, optimizer, scheduler, test_loader, 11, eval = True)
  print (test_metrics['total'], test_metrics['accuracy'], test_metrics['loss'])
...
model = CrossEncoderModel (model_checkpoint, num_classes)
(model, test_metrics) = model_training (model, 20, tokenized_dataset, 256)
